...
class Custom_Meta_Variable_Input { 
	...
	public function save_variables(int $variation_id, int $loop){
    	...
    	$Base_Meta->set_minQuantity((int)$post[Base_Custom_Meta::MIN_QUANTITY][$loop] ?: 1);
    	$Base_Meta->set_incrementStep((int)$post[Base_Custom_Meta::INCREMENT_STEP][$loop] ?: 1);
    ...
    public function render_Variable_fields($loop, $variation_data, $variation) {
        ...
        woocommerce_wp_text_input(array(
            ...
            'label'         => __('Unit Purchase Price', 'Peleman_Editor_Communicator'),
        ...
        woocommerce_wp_text_input(array(
            ...
            'label'             => __('Unit amount', 'Peleman_Editor_Communicator'),
            'value'             => (string)$Base_Meta->get_cartUnits() ?: 1,
            'desc_tip'          => true,
            'description'       =>  __('Amount of items per unit. ie. 1 box (unit) contains 20 cards (items).', 'Peleman_Editor_Communicator'),
        ...
        woocommerce_wp_text_input(array(
            ...
            'label'         => __('Unit code', 'Peleman_Editor_Communicator'),
            'value'         => (string)$Base_Meta->get_unitCode(),
            'desc_tip'      => true,
            'description'   =>  __('The unit code for internal identification , ie. BOX, CRT, ...', 'Peleman_Editor_Communicator'),
            'wrapper_class' => 'form-row form-row-first wsppe-form-row-padding-5',
            'placeholder'   => 'BOX, CRT, ...'
        ));
        ...
        woocommerce_wp_text_input(array(
            ...
            'label'         => __('Custom add to cart label', 'Peleman_Editor_Communicator'),
            'value'         => $Base_Meta->get_customAddToCartLabel(),
            'desc_tip'      => true,
            'description'   =>  __('Custom Add To Cart label that will be displayed on the product page', 'Peleman_Editor_Communicator'),
            'wrapper_class' => 'form-row form-row-last wsppe-form-row-padding-5',
		...
		woocommerce_wp_text_input(array(
            ...
            'label'             => __('Minimum Quantity', 'Peleman_Editor_Communicator'),
            'value'             => $Base_Meta->get_minQuantity(),
            'desc_tip'          => true,
            'description'       =>  __('Set the minimum quantity of the order', 'Peleman_Editor_Communicator'),
            'wrapper_class'     => 'form-row form-row-first wsppe-form-row-padding-5',
            'type'              => 'number', 
			'custom_attributes' => array(
                'step'              => 1,
		...
		woocommerce_wp_text_input(array(
            ...
            'label'             => __('increment step', 'Peleman_Editor_Communicator'),
            'value'             => $Base_Meta->get_incrementStep(),
            'desc_tip'          => true,
            'description'       =>  __('Set the steps of the increment', 'Peleman_Editor_Communicator'),
            'wrapper_class'     => 'form-row form-row-last wsppe-form-row-padding-5',
            'type'              => 'number', 
			'custom_attributes' => array(
                'step'              => 1,
